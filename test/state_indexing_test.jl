...
function test_no_boundschecks(fi::FakeIntegrator)
    test_no_boundschecks(fi.u)
    test_no_boundschecks(fi.p)
end

@testset "FakeIntegrator: inbounds = $inbounds" for inbounds in [false, true]
    fi = FakeIntegrator(sys, maybe_CCA(copy(u), inbounds), maybe_CCA(copy(p), inbounds), t)
    ...
    for (sym, val, newval, check_inference) in [(:x, u[1], 4.0, true)
        ...
        setter = setsym_oop(sys, sym; inbounds)
    ...
    for (sym, oldval, newval, check_inference) in [(:a, p[1], 4.0, true)
        ...
        oop_setter = setsym_oop(sys, sym; inbounds)
...
@testset "FakeSolution: inbounds = $inbounds" for inbounds in [false, true]
    ...
    yvals = getindex.(u, 2)
    zvals = getindex.(u, 3)
    sol = FakeSolution(sys, maybe_CCA(maybe_CCA.(u, inbounds), inbounds),
        maybe_CCA(p, inbounds), maybe_CCA(t, inbounds))
    ...
    let sol = sol, sys = sys
        ...
        @test getter(sol) == [() for _ in 1:length(sol.t)]
        sc = SymbolCache(nothing, [:a, :b], :t)
        sol = FakeSolution(sys, maybe_CCA([], inbounds),
            maybe_CCA([1.0, 2.0], inbounds), maybe_CCA([], inbounds))
        getter = getsym(sc, [])
        @test getter(sol) == []
        getter = getsym(sc, ())
        @test getter(sol) == []
    ...
    u = [1.0, 2.0, 3.0]
    p = [10.0, 20.0, 30.0]
    fs = FakeSolution(sys, maybe_CCA(u, inbounds), maybe_CCA(p, inbounds), nothing)
...
@testset "NonMarkovian: inbounds = $inbounds" for inbounds in [false, true]
    fi = FakeIntegrator(sys, maybe_CCA(u0, inbounds), maybe_CCA(p, inbounds), ts[1])
    fs = FakeSolution(sys, maybe_CCA(maybe_CCA.(u, inbounds), inbounds),
        maybe_CCA(p, inbounds), maybe_CCA(ts, inbounds))
    getter = getsym(sys, :(x + y); inbounds)
    @test getter(fi) ≈ 2.8
    ...
    pstate = ProblemState(; u = maybe_CCA(u0, inbounds), p = maybe_CCA(p, inbounds),
        t = ts[1], h = t -> t .* ones(length(u0)))
    @test getter(pstate) ≈ 2.8

    if inbounds
        test_no_boundschecks(pstate)
    end

    @testset "Tuple observed" begin
        sc = SymbolCache([:x, :y, :z], [:a, :b, :c])
        sys = TupleObservedWrapper(sc)
        ps = ProblemState(; u = maybe_CCA([1.0, 2.0, 3.0], inbounds),
            p = maybe_CCA([0.1, 0.2, 0.3], inbounds))
        getter = getsym(sys, (:(x + y), :(y + z)); inbounds)
        @test all(getter(ps) .≈ (3.0, 5.0))
        @test getter(ps) isa Tuple
        @test_nowarn @inferred getter(ps)
        getter = getsym(sys, (:(a + b), :(b + c)); inbounds)
        @test all(getter(ps) .≈ (0.3, 0.5))
        @test getter(ps) isa Tuple
        @test_nowarn @inferred getter(ps)
        if inbounds
            test_no_boundschecks(ps)
        end

        sc = SymbolCache([:x, :y, :z], [:a, :b, :c], :t)
        sys = TupleObservedWrapper(sc)
        ps = ProblemState(; u = maybe_CCA([1.0, 2.0, 3.0], inbounds),
            p = maybe_CCA([0.1, 0.2, 0.3], inbounds), t = 0.1)
        getter = getsym(sys, (:(x + y), :(y + t)); inbounds)
